...
def currency_coefficient(row, currency, df_currency):
    if currency == "RUR":
        return 1
...
def get_salary(row, df_currency):
    salary_to = row['salary_to']
    salary_from = row['salary_from']
    if pd.isna(row['salary_currency']):
        return None
    coefficient = currency_coefficient(row, row["salary_currency"], df_currency)
...
if __name__ == "__main__":
    ...
    csv_merged['salary'] = csv_merged.apply(lambda row: get_salary(row, df_currency), axis=1)
